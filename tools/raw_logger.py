...
def log_raw_telemetry():
    ...
    logger.info(f"Mode: {mode_label} | Polling Rate: {polling_rate} Hz | Target Port: {args.port} | Mock: {args.mock}")
    ...
    interval = 1.0 / max(0.01, polling_rate)

    print("\n" + "=" * 80)
    print(f"  🔴 LOGGING RAW YAMAHA YDS ECU TELEMETRY -> {args.output}")
    print(f"  Mode: {mode_label} ({len(target_opcodes)} Opcodes)")
        ...
        with open(args.output, "a", encoding="utf-8") as outfile:
            ...
            while True:
                ...
                if args.mock:
                    ...
                    sim_rpm = int(650.0 + math.sin(sim_t * 0.5) * 50.0)
                    raw_vals = {
                        0x00: (sim_rpm >> 8) & 0xFF,
                        0x01: sim_rpm & 0xFF,
                        0x02: 1,
